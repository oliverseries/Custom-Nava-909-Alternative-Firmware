...
void SavePattern(byte patternNbr) {
  ...
  //PROB-----------------------------------------------
  int offset = patternNbr * 256;
  for (byte i = 0; i < NBR_STEP; i++) {
    int stepOffset = i * 64;
    for (byte j = 0; j < NBR_INST; j++) {
      EEPROM.write(offset + stepOffset + j, pattern[ptrnBuffer].stepProb[i][j]);
    }
  }

  //REPEAT-----------------------------------------------
  int repeatOffset = (patternNbr * 256)+4096;
  for (byte i = 0; i < NBR_STEP; i++) {
    EEPROM.write(repeatOffset + i, pattern[ptrnBuffer].stepFlamRpts[i]);
  }
}
...
void LoadPattern(byte patternNbr) {
  ...
  for (byte i = 0; i < NBR_STEP; i++) {
    int stepOffset = i * 64;
    for (byte j = 0; j < NBR_INST; j++) {
      if (EEPROM.read(offset + stepOffset + j) == 255) {
        pattern[!ptrnBuffer].stepProb[i][j] = 0;
      } else {
        pattern[!ptrnBuffer].stepProb[i][j] = EEPROM.read(offset + stepOffset + j);
      }
    }
  }
  ...
  int repeatOffset = (patternNbr * 256)+4096;
  for (byte i = 0; i < NBR_STEP; i++) {
    if (EEPROM.read(repeatOffset + i) == 255) {
      pattern[!ptrnBuffer].stepFlamRpts[i] = 0;
    } else {
      pattern[!ptrnBuffer].stepFlamRpts[i] = EEPROM.read(repeatOffset + i);
    }
  }
