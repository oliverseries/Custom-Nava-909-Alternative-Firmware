...
//    dinClkState = HIGH;
  ...
  if (curSeqMode == PTRN_STEP || curSeqMode == PTRN_TAP){
    ...
    if (clearBtn.justPressed && !keyboardMode && curSeqMode != PTRN_TAP && !isRunning){
      ...
      for (int c = 0; c < NBR_STEP; c++) {
        pattern[ptrnBuffer].stepFlamRpts[c] = 0;
        for (int d = 0; d < NBR_INST; d++) {
          pattern[ptrnBuffer].stepProb[c][d] = 0;
        }
      }
    ...
    if (curSeqMode == PTRN_STEP){
      ...
      if(!lastStepBtn.pressed && !instBtn && !keyboardMode && !shufBtn.pressed){                                      // [zabox] test
        if (isRunning)
        {
          ...
          if (stepsBtn.pressed && encBtn.pressed && curFlam) {
            lcd.clear();
            LcdUpdate();
          }
          if (stepsBtn.pressed && encBtn.justRelease && curFlam) {
            encoderPressFlamMenu();
          }
